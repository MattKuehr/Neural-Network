...
'''
Code to normalize numpy methods across machines
See https://github.com/Sentdex/nnfs/blob/master/nnfs/core.py for more information
'''


# Initializes NNFS
def np_init(dot_precision_workaround=True, default_dtype='float32', random_seed=0):
...
# Call numpy override function to standardize numpy methods across machines
#np_init()
# handled in driver instead ^^
...
class CustomCSVReader:
    def __init__(self, filepath, delimiter=","):
        ...
        self.data, self.column_names = self._read_file()

    def _read_file(self):
        # Open and read the file
        with open(self.filepath, "r") as f:
            # Read the first line for column names
            column_names = f.readline().strip().split(self.delimiter)
        ...
        data = np.array(data, dtype=float)  # Assuming numeric data; adjust dtype as needed
        return data, column_names

    def get_column(self, column_name, flatten=True):
        # Find the index of the column name
        if column_name not in self.column_names:
            raise ValueError(f"Column '{column_name}' not found in CSV.")
        col_idx = self.column_names.index(column_name)
        
        if flatten==True: # For classification
            # Return the specified column as a 1D array
            return self.data[:, col_idx]
        
        if flatten==False: # For regression
            return np.array([[row[col_idx]] for row in self.data])

    def get_columns(self, column_names):
        # Check if all column names are valid
        col_indices = [self.column_names.index(name) for name in column_names if name in self.column_names]
        ...
        # Return the specified columns as a 2D array
        return self.data[:, col_indices]


# From-scratch Train-Test-Split
...
def normalize(X_train, X_test, X_val=None, has_val=False):
...
class Layer_Dense:
    ...
    def __init__(self, n_inputs, n_neurons, scaler = 0.01, init_method='random',
                weight_regularizer_l1=0, weight_regularizer_l2=0,
                bias_regularizer_l1=0, bias_regularizer_l2=0):

        # Random initialization (default)
        if init_method == 'random':
            self.weights = scaler * np.random.randn(n_inputs, n_neurons)
            self.biases = np.zeros((1, n_neurons))
        
        # Uniform distribution
        elif init_method == 'uniform':
            limit = 1 / np.sqrt(n_inputs)
            self.weights = scaler * np.random.uniform(-limit, limit, size=(n_inputs, n_neurons))
            self.biases = np.zeros((1, n_neurons))
        ...
        # He initialization (normal) (fan_in)
        elif init_method == 'he':
            # fan_in mode (there is also fan_out and fan_avg)
            std_dev = np.sqrt(2 / n_inputs)
            self.weights = scaler * np.random.normal(0, std_dev, size=(n_inputs, n_neurons))
            self.biases = np.zeros((1, n_neurons))
        
        # LeCun initialization (normal)
        elif init_method == 'lecun':
            std_dev = 1 / np.sqrt(n_inputs)
            self.weights = scaler * np.random.normal(0 , std_dev, size=(n_inputs, n_neurons))
            self.biases = np.zeros((1, n_neurons))

        else:
            bad_init = ("Invalid weight initialization selected, try: \n"
            "\'random\' \n"
            "\'uniform\' \n"
            "\'glorot\' \n"
            "\'he\' \n"
            "\'lecun\' \n"
            "Be cautious with your selection of \'scaler\' for these methods.")

            raise Exception(bad_init)
...
class Activation_Linear:
    ...
    def backward(self, dvalues):
        ...
        # Derivative of 'x' is 1, 1 * dvalues = dvalues by chain rule
        self.dinputs = dvalues.copy()
...
class Loss:
    ...
    def regularization_loss(self):
        ...
        # Calculate regularization loss, iterating all trainable layers
    ...
    # Calculates the data and regularizationlosses
...
class Loss_BinaryCrossentropy(Loss):
    ...
    def backward(self, dvalues, y_true):
        ...
        # Normalize gradient to make it batch size invariant
        self.dinputs = self.dinputs / samples
...
class Loss_MeanSquaredError(Loss): # L2 loss
    ...
    def backward(self, dvalues, y_true):
        ...
        # Identify number of outputs associated with each sample
        # Checking first sample to count outputs
        outputs = len(dvalues[0])

        # Gradient on values
        self.dinputs = -2 * (y_true - dvalues) / outputs

        # Normalize gradient to be sample invariant
        self.dinputs = self.dinputs / samples
...
class Loss_MeanAbsoluteError(Loss): # L1 loss
    ...
    def backward(self, dvalues, y_true):
        ...
        # Identify number of outputs in each sample
        # Count outputs from first sample to do this
        outputs = len(dvalues[0])
...
class Accuracy:
    ...
    def calculate_accumulated(self):
        ...
        # Return the accuracy for given accumulation 
        # period (step / epoch)
        return accuracy
...
class Accuracy_Regression(Accuracy):
    ...
    # Calculate precision value based on ground truth values
    def init(self, y, reinit=False):
        if self.precision is None or reinit:
            self.precision = np.std(y) / 3
...
class Model:
    ...
    def finalize(self):
        ...
        if self.loss is not None:
            ...
            # Remember trainable layers if training, otherwise
            # we are likely using pre-determined weights
    ...
    def backward(self, output, y):
        ...
        if self.softmax_classifier_output is not None:
            ...
            # First call backward method on
            # the combined activation / loss
    ...
    def set_parameters(self, parameters):
        ...
        # and update each layer with each set of 
        # weights and biases
...
'''
Fashion MNIST dataset functions
'''
